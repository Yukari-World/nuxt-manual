...
.category--scss.page--gradation-list
	...
	section
		h2 コードの生成
		h3 pug
		BlockCode.language-pug: pre {{ CBPug1 }}

		p これでも良いが、リストの追加や配置の入れ替え等を効率良く行えるようにするため以下のように記述する。
		BlockCode.language-pug: pre {{ CBPug2 }}
		...
		p
			...
			| を参照。
		BlockCode.language-scss: pre {{ CBScss }}

	section
		h2 コードの出力結果
		h3 HTML
		BlockCode.language-html: pre {{ CBResultHTML }}
		...
		p
			...
			| により自動で確認、付与されている。
		BlockCode.language-css: pre {{ CBResultCSS }}
</template>
...
const CBScss = ref(`@charset "UTF-8";
...
const CBResultHTML = ref(`<div class="list">
	<ul>
		<li class="num-1">
