...
div
	...
	section
		...
		h3 マニュアルの互換性低くない?
		p
			| 最先端技術を取り扱うのを優先しているため、コーディングの互換性は大幅に犠牲になっています。互換性のある記述方法も順次記述していきますが、いつになるかは不明です。

		h3 人によってこのページが白い(黒い)のは何故?
		p
			| このサイトではカラーテーマが有効になっているので今使っているブラウザのテーマがライトかダークかによって画面が変わってきます。
			br
			| Windowsのブラウザの場合、Windowsにあるテーマ側のダークモードが有効かどうかに委ねられることが多いので、初期設定でライトモードである場合が多いです。
			br
			| 印刷を行う場合、インクの消費を抑える目的でライトモードと同等のテーマと色彩になるように強制されます。
			br
			| 尚、この仕様が無効なブラウザの場合、デフォルトでダークテーマが適応されます。

		h3 フッターにある取り消し線の内容は何?
		p
			| 全て元ネタがあります。わからない場合、調べてみましょう。尚、内容は秒が10の倍数毎に切り替わるようになっています。現在
			span#countRandom ---
			| 個の候補があります。表示内容は
