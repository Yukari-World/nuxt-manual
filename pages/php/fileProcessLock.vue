...
.category--php.page--file-process-lock
	...
	section
		h2  専有ロックとの違い
		p 専有ロックは複数のユーザーがそのスクリプトにアクセスし、他の人がそのファイルを読み書きしている場合、読み書きを一時的に待機してもらい、ロックが解除した時にアクセスを続行する処理である。
			br
	...
	section
		...
		BlockCode.language-php {{ CB01 }}

	section
		h2 使用上の注意
		ul
			li 処理が異常終了しロックファイルが残留してしまった場合、このスクリプトはアクセスできなくなるのでロック処理の例外処理が必要である。
				br
