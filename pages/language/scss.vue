...
.category--language.page--scss
	...
	section
		...
		h4 SCSS
		BlockCode.language-scss {{ CBScssSample }}
	...
	section
		...
		h4 SCSS
		BlockCode.language-scss {{ CBScssVariable }}
		...
		p
			| タグ内に
			TextToken(type="scss").token.atrule @media
			| タグを記述することで簡単にレスポンシブ対応ができる。
		h4 SCSS
		BlockCode.language-scss {{ CBScssMedia01 }}
		...
		h4 SCSS
		BlockCode.language-scss {{ CBScssMedia02 }}
	...
	section
		h2 変換方法
		p Scssはテンプレートエンジンであるため、そのままではCSSとして認識されない。そのため変換する必要がある。これはその一例である。
		h3 Vueで使用
		...
		ol
			...
			li
				...
				BlockCode.language-markup {{ CBUseScss }}
